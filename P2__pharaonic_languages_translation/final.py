...
@smart_inference_mode()
def run_2(
        weights_2=ROOT / 'runs/train/exp8/weights/best.pt',  # model path or triton URL
        source_2=ROOT / 'sec_roi.jpg',  # file/dir/URL/glob/screen/0(webcam)
        data_2=ROOT / 'data_4.yaml',  # dataset.yaml path
    ...
    model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half)
    stride, names, pt = model.stride, model.names, model.pt
    imgsz_2 = check_img_size(imgsz_2, s=stride)  # check image size

    # Dataloader
    bs = 1  # batch_size
    if webcam:
        view_img = check_imshow(warn=True)
        dataset = LoadStreams(source_2, img_size_2=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
        bs = len(dataset)
    elif screenshot:
        dataset = LoadScreenshots(source_2, img_size_2=imgsz, stride=stride, auto=pt)
    else:
        dataset = LoadImages(source_2, img_size_2=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
    vid_path, vid_writer = [None] * bs, [None] * bs
...
def parse_opt_2():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights_2', nargs='+', type=str, default=ROOT / 'runs/train/exp8/weights/best.pt', help='model path or triton URL')
    parser.add_argument('--source_2', type=str, default=Null, help='file/dir/URL/glob/screen/0(webcam)')#ROOT / 'data/images'
    parser.add_argument('--data_2', type=str, default=ROOT / 'data_4.yaml', help='(optional) dataset.yaml path')
